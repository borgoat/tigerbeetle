    ...
    fn flush_io(_: *IO, events: []os.Kevent, io_pending_top: *?*Completion) usize {
        for (events) |*event, flushed| {
            ...
            const event_info = switch (completion.operation) {
                .accept => |op| [2]c_int{ op.socket, os.E.VFILT.READ },
                .connect => |op| [2]c_int{ op.socket, os.E.VFILT.WRITE },
                .read => |op| [2]c_int{ op.fd, os.E.VFILT.READ },
                .write => |op| [2]c_int{ op.fd, os.E.VFILT.WRITE },
                .recv => |op| [2]c_int{ op.socket, os.E.VFILT.READ },
                .send => |op| [2]c_int{ op.socket, os.E.VFILT.WRITE },
            ...
            event.* = .{
                ...
                .flags = os.E.V.ADD | os.E.V.ENABLE | os.E.V.ONESHOT,
                .fflags = 0,
                .data = 0,
        ...
        self.submit(
            ...
            struct {
                fn doOperation(op: anytype) AcceptError!os.socket_t {
                    ...
                    // Darwin doesn't support os.MSG.NOSIGNAL to avoid getting SIGPIPE on socket send().
